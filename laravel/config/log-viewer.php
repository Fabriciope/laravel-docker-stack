...
return [
    ...
    */
    ...
    // A autorização é tratada pelo middleware App\Http\Middleware\LogViewerBasicAuth
    // (Basic Auth com credenciais dedicadas), então o guard interno do pacote fica
    // desligado para não interferir.
    'require_auth_in_production' => false,

    /*
    |--------------------------------------------------------------------------
    | Credenciais de acesso (HTTP Basic Auth)
    |--------------------------------------------------------------------------
    | Usadas pelo LogViewerBasicAuth. Defina ambas no .env de produção para
    | proteger /log-viewer. Se ficarem vazias: liberado em dev, bloqueado em prod.
    |
    */

    'basic_auth' => [
        'user' => env('LOG_VIEWER_AUTH_USER'),
        'password' => env('LOG_VIEWER_AUTH_PASSWORD'),
    ],
    ...
    */
    ...
    'include_files' => [
        // Varre todos os .log de storage/logs, incluindo os subdiretórios
        // nginx/ e php-fpm/ (bind-montados nos containers — ver docker-compose).
        // O tipo de cada arquivo é autodetectado pelo conteúdo:
        //   nginx/access.log  -> "HTTP Access"        (formato combined padrão do nginx)
        //   nginx/error.log   -> "HTTP Error (Nginx)"
        //   php-fpm/php-fpm.log -> "PHP-FPM"
        //   laravel.log       -> "Laravel"
        // (O access.format customizado do php-fpm em www.access.log não tem parser
        //  embutido e fica como "Unknown" — escondido por hide_unknown_files abaixo.)
        '*.log',
        '**/*.log',
    ],
