    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 *
                 * NOTE: We deliberately do NOT back up base_path() — the application code
                 * lives in git, and base_path() would pull the `.env` (secrets) into the
                 * archive. We only keep locally-stored user content here. The scheduled
                 * daily backup runs with `--only-db` and skips files entirely anyway.
                 */
                'include' => [
                    storage_path('app'),
                ],
        ...
        'destination' => [
             ...
             *
             * Local-only by design (see PLANO §14.6). The `backups` disk maps to a host
             * bind mount (storage/app/backups) in production, so archives survive deploys
             * and stay reachable on the host. Note: a local-only backup does NOT protect
             * against host loss — to close that gap later, replicate this directory
             * off-host (rsync/rclone) or add an S3-compatible disk here.
             */
            'disks' => [
                'backups',
            ],
